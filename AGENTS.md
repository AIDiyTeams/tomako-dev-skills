...
本目录是 **Tomako 团队工程协作 Skills** 的 canonical 来源。在 `tomako-workspace` 根目录打开 Cursor 时，先运行 `./tomako-dev-skills/install.sh` 将 skills 链接到 `.cursor/skills/`。
...
3. 所有路径以 **tomako-workspace 根目录** 为基准，不要假设 skill 在 `Tomako/` 子目录内
